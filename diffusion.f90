...
SUBROUTINE diffusion(phi,lewis,dtime,Q,ng,nglow,dx,dy,mynx,myny,myid,&
  ...
  REAL(WP) :: h,h2,tol,tolphi,errphi,tolphi_tot,errphi_tot
! 
  ! Allocate space for FV coefficients 
  allocate(ae(ng),aw(ng),an(ng),as(ng))
  ! Calculate finite volume coefficients
  call calc_ad(ae,aw,an,as,lewis,ng,ng,dx,dy,mynx,myny)
  ...
  do j=2,myny(ng)-1
     do i=2,mynx(ng)-1
        rhs(i,j)=0.5_wp*(ae(ng)%a(i-1,j-1)*phi(i+1,j)+&
             ...
             as(ng)%a(i-1,j-1)*phi(i,j-1))+(h2/dtime - &
             0.5_wp*(ae(ng)%a(i-1,j-1)+aw(ng)%a(i-1,j-1)+&
             an(ng)%a(i-1,j-1)+as(ng)%a(i-1,j-1)))*phi(i,j)+h2*Q
     enddo
  enddo
...
!
CONTAINS
  SUBROUTINE calc_ad(ae,aw,an,as,le,ng,nglow,dx,dy,mynx,myny)
    USE btype
    USE bfunc2d
    IMPLICIT NONE 
    ...
    forall (i=1:mynx(ng)-2,j=1:myny(ng)-2)
       aw(ng)%a(i,j)=(h/le)/dx(ng)%d(i)
       ae(ng)%a(i,j)=(h/le)/dx(ng)%d(i+1)
       as(ng)%a(i,j)=(h/le)/dy(ng)%d(j)
       an(ng)%a(i,j)=(h/le)/dy(ng)%d(j+1)
    end forall
