...
SUBROUTINE nbrex2d_int(ue,tfsl,bfsl,lfsl,rfsl,myid,nbrleft,nbrright,&
  ...
  call MPI_SENDRECV(ue(2:nx1-1,ny1-1),nx1-2,&
       MPI_INTEGER,nbrtop,0,ue(2:nx1-1,1),&
       nx1-2,MPI_INTEGER,nbrbottom,0,&
       comm2d,status,ierr)
  call MPI_SENDRECV(ue(2:nx1-1,2),nx1-2,&
       MPI_INTEGER,nbrbottom,1,ue(2:nx1-1,ny1),&
       nx1-2,MPI_INTEGER,nbrtop,1,&
       comm2d,status,ierr)
  ! Now can send entire columns in x direction 
  ! (including ghost points in y)
  call MPI_SENDRECV(ue(nx1-1,1:ny1),ny1,&
       MPI_INTEGER,nbrright,2,ue(1,1:ny1),&
       ny1,MPI_INTEGER,nbrleft,2,&
       comm2d,status,ierr)
  call MPI_SENDRECV(ue(2,1:ny1),ny1,&
       MPI_INTEGER,nbrleft,3,ue(nx1,1:ny1),&
       ny1,MPI_INTEGER,nbrright,3,&
       comm2d,status,ierr)
