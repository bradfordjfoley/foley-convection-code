...
!!$     enddo
     do i=1,64
        x_tot(i)=(0.4_wp/64)*i-0.5_wp*(0.4_wp/64)
     enddo
     do i=1,128
        x_tot(i+64)=(0.2_wp/128)*i-0.5_wp*(0.2_wp/128)+0.4_wp
     enddo
     do i=1,64
        x_tot(i+128+64)=(0.4_wp/64)*i-0.5_wp*(0.4_wp/64)+0.6_wp
     enddo
     do i=1,64
        y_tot(i)=(0.4_wp/64)*i-0.5_wp*(0.4_wp/64)
     enddo
     do i=1,128
        y_tot(i+64)=(0.2_wp/128)*i-0.5_wp*(0.2_wp/128)+0.4_wp
     enddo
     do i=1,64
        y_tot(i+128+64)=(0.4_wp/64)*i-0.5_wp*(0.4_wp/64)+0.6_wp
     enddo
  endif
...
!!$  enddo
  do i=1,64
     xu_tot(i)=(0.4_wp/64)*(i-1)
  enddo
  do i=65,193
     xu_tot(i)=(0.2_wp/128)*(i-65)+0.4_wp
  enddo
  do i=194,257
     xu_tot(i)=(0.4_wp/64)*(i-194)+0.6_wp
  enddo
  do i=1,64
     yw_tot(i)=(0.4_wp/64)*(i-1)
  enddo
  do i=65,192
     yw_tot(i)=(0.2_wp/128)*(i-65)+0.4_wp
  enddo
  do i=193,257
     yw_tot(i)=(0.4_wp/64)*(i-193)+0.6_wp
  enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  elseif (ibc==8) then
     ...
     forall (i=1:mynx(ng),j=1:myny(ng))
        temp(i,j)=((tanh((x(ng)%d(i)-0.4_wp)*50.0_wp)+1.0_wp)*0.5_wp)*&
             (1.0_wp-(tanh((x(ng)%d(i)-0.6_wp)*50.0_wp)+1.0_wp)*0.5_wp)*&
             ((tanh((y(ng)%d(j)-0.4_wp)*50.0_wp)+1.0_wp)*0.5_wp)*&
             (1.0_wp-(tanh((y(ng)%d(j)-0.6_wp)*50.0_wp)+1.0_wp)*0.5_wp)
     end forall
     chem=0.0_wp
     press=0.0_wp
     u=1.0_wp
...
!ntracers is greater than 0; otherwise it is done with the grid based scheme
  ...
  elseif (chem_layer_init.and.ntracers>0) then
     ...
     call calc_chem_tr_ratio(tracers,chem,x,yw,myntracers,asp,nx,ny,sx,syw,&
...
!!$                   alpha(i,j)**(-m_alpha)
!!$           endif
!!$           if ((0.5_wp*chem(i,j-1)+0.5_wp*chem(i,j)).gt.0.75_wp) then
!!$              mu(i,j)=mu(i,j)*mu_jump
!!$           endif
!!$        enddo
!!$     enddo
...
!!$        call MPI_ALLREDUCE(errv,errv_tot,1,MPI_DOUBLE_PRECISION,&
!!$             MPI_MAX,comm2d,ierr)
!!$        tolv_tot=10.0_wp*max(tolu_tot/maxval(dxu(ng)%d),&
!!$             tolw_tot/maxval(dyw(ng)%d))
      ...
      do n=1,myntracers
         ...
         if (tracers(n)%d(1)==1.0_wp.and.nx1/=nx2.and.ny1/=ny2) then
            n_tr_weight_dense(nx1,ny1)=n_tr_weight_dense(nx1,ny1)+&
                 ...
                 (abs(tracers(n)%d(3)-y(ng)%d(ny1)))/(y(ng)%d(nx2)-&
                 y(ng)%d(nx1)))
            ...
            n_tr_weight_dense(nx1,ny2)=n_tr_weight_dense(nx1,ny2)+&
                 max(0.0_wp,1.0_wp-(abs(tracers(n)%d(2)-x(ng)%d(nx1)))/&
                 (x(ng)%d(nx2)-x(ng)%d(nx1)))*max(0.0_wp,1.0_wp-&
                 (abs(tracers(n)%d(3)-y(ng)%d(ny2)))/(y(ng)%d(nx2)-&
                 y(ng)%d(nx1)))
