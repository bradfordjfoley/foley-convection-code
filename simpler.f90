...
PROGRAM simpler 
  ...
  REAL(WP) :: t1,t2,erra,h,h2,lbvt,rbvt,tbvt,bbvt,dtime,dt,dta,dtd,time,bn,&
       ...
       errv_tot,errv,maxdu,maxdu_tot,maxdw,maxdw_tot,tolv_tot,u0,&
  ...
  INTEGER :: n,i,j,k,l,ierr,myid,numprocs,npx,npy,comm2d,ng2,n2,&
       ...
       i_start,i_end,j_start,j_end,filenumber_tr,ntracers_init
  ...
  call mkgrid(sx,ex,sy,ey,asp,ng,nglow,lhbdy,lhbct,lhbcw,lhbcc,rhbdy,rhbct,&
       ...
       mynx,myny,x,xw,xc,y,yu,yc,dx,dxw,dxc,dy,dyu,dyc)
  ...
  if (myid==0) then
     allocate(sxall(numprocs),syall(numprocs),mynxall(numprocs),&
          ...
          xu_tot(nx+1),yw_tot(ny+1),eyall(numprocs),eywall(numprocs))
  endif
  ...
  myxmin=x(ng)%d(2)-0.5_wp/(nx/asp)
  myxmax=x(ng)%d(mynx(ng)-1)+0.5_wp/(nx/asp)
  myymin=y(ng)%d(2)-0.5_wp/ny
  myymax=y(ng)%d(myny(ng)-1)+0.5_wp/ny
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        do i=2,mynx(ng)-1
           do j=2,myny(ng)-1
              do k=1,tracers_per_cell
                 ...
                 seed=myid+seed_tracers*seed_tracers*l
                 call random_seed(PUT=seed)
                 ! Initial x,y position for this single tracer test case
                 call random_number(rn)
                 call random_number(rn)
                 tracers(l)%d(2)=x(ng)%d(i)+deltax*(rn-0.5_wp)
                 call random_number(rn)
                 tracers(l)%d(3)=y(ng)%d(j)+deltay*(rn-0.5_wp)
                 l=l+1
              enddo
           enddo
        enddo
  ...
  if (ibc==1) then
     ...
     if (botbdy) then
        ...
        do i=2,mynx(ng)-1
           call random_number(rn)
           temp(i,myny(ng)/2)=temp(i,myny(ng)/2) + 2E-2*(0.5_wp-rn) 
        enddo
     endif
     if (ntracers==0) then
        chem=0.0_wp
...
!ntracers is greater than 0; otherwise it is done with the grid based scheme
  ...
  elseif (chem_layer_init.and.ntracers>0) then
     ...
     call calc_chem_tr_ratio(tracers,chem,x,yw,myntracers,nx,ny,sx,syw,&
...
!!$     end forall
     ...
     do iter=1,iter_total
        ...
        forall (i=2:mynx(ng)-1,j=2:mynyw(ng)-1)
           w_rhs(i,j)=-Ra*(0.5_wp*(temp(i,j)+temp(i,j+1))-&
                Bu*(chem(i,j)))-1.0_wp/h2*(mu_c(i,j)*(u(i,j+1)-u(i,j))&
        ...
        forall (i=2:mynx(ng)-1,j=2:mynyw(ng)-1)
           w_hat(i,j)=(1.0_wp/(aew(ng)%a(i-1,j-1)+aww(ng)%a(i-1,j-1)+&
                ...
                *w(i+1,j)+aww(ng)%a(i-1,j-1)*w(i-1,j)+anw(ng)%a(i-1,j-1)*&
                w(i,j+1)+asw(ng)%a(i-1,j-1)*w(i,j-1)-h2*w_rhs(i,j))
        end forall
        ! BC's 
        ...
        forall (i=2:mynxu(ng)-1,j=2:myny(ng)-1)
           u_rhs(i,j)=-1.0_wp/h2*(mu_c(i,j)*(w(i+1,j)-w(i,j))&
        ...
        forall (i=2:mynxu(ng)-1,j=2:myny(ng)-1)
           u_hat(i,j)=1.0_wp/(aeu(ng)%a(i-1,j-1)+awu(ng)%a(i-1,j-1)+&
                ...
                u(i,j+1)+asu(ng)%a(i-1,j-1)*u(i,j-1)-h2*u_rhs(i,j))
        end forall
        ! BC's       
        ...
        forall (i=2:mynx(ng)-1,j=2:myny(ng)-1)
           p_rhs(i,j)=1.0_wp/h*(u_hat(i,j)-u_hat(i-1,j))+&
                1.0_wp/h*(w_hat(i,j)-w_hat(i,j-1))
        end forall
        ...
        do loop=1,iter_uw_loop
           ...
           ! For tracer method with composition set on same grid as w
           forall (i=2:mynx(ng)-1,j=2:mynyw(ng)-1)
              w_rhs(i,j)=1.0_wp/h*(press(i,j+1)-press(i,j))&
                   -Ra*(0.5_wp*(temp(i,j)+temp(i,j+1))-&
                   Bu*(chem(i,j)))-1.0_wp/h2*(mu_c(i,j)*(u(i,j+1)-u(i,j))&
                   -mu_c(i-1,j)*(u(i-1,j+1)-u(i-1,j))) 
           end forall
           ...
           ! Set up rhs for u
           forall (i=2:mynxu(ng)-1,j=2:myny(ng)-1)
              u_rhs(i,j)=1.0_wp/h*(press(i+1,j)-press(i,j))&
                   -1.0_wp/h2*(mu_c(i,j)*(w(i+1,j)-w(i,j))&
                   -mu_c(i,j-1)*(w(i+1,j-1)-w(i,j-1)))
           end forall
        ...
        forall (i=2:mynx(ng)-1,j=2:myny(ng)-1)
           p_rhs(i,j)=(1.0_wp/h)*(u(i,j)-u(i-1,j))+(1.0_wp/h)*(w(i,j)-w(i,j-1))
        end forall
        p_cor=0.0_wp
...
!!$             0.0_wp,comm2d)
        ...
        forall (i=2:mynxu(ng)-1,j=2:myny(ng)-1)
           u(i,j)=u(i,j)+(ae_p(ng)%a(i-1,j-1)/h)*(p_cor(i,j)-p_cor(i+1,j))
        end forall
        call nbrex2d(u,utfsl,ubfsl,.false.,.false.,myid,nbrleft,&
             nbrright,nbrtop,nbrbottom,comm2d)
        forall (i=2:mynx(ng)-1,j=2:mynyw(ng)-1)
           w(i,j)=w(i,j)+(an_p(ng)%a(i-1,j-1)/h)*(p_cor(i,j)-p_cor(i,j+1))
        end forall
        ...
        if (remove_net_u) then
           ...
           call average_u(u,deltay,deltax,asp,u0,mynx,myny,ng)
           u=u-u0
        endif
        ...
        call MPI_ALLREDUCE(errw,errw_tot,1,MPI_DOUBLE_PRECISION,&
             MPI_MAX,comm2d,ierr)
        tolp=tol*(maxval(abs(press)))
        if (tolp.eq.0.0) tolp=tol
        ...
        call MPI_ALLREDUCE(errp,errp_tot,1,MPI_DOUBLE_PRECISION,&
             MPI_MAX,comm2d,ierr)
        errv=maxval(abs(h*p_rhs))
        call MPI_ALLREDUCE(errv,errv_tot,1,MPI_DOUBLE_PRECISION,&
             MPI_MAX,comm2d,ierr)
        tolv_tot=10.0_wp*max(tolu_tot,tolw_tot)
...
!!!!!!!!!!!!!!!!!!!!!WRITE OUT SOLUTION FOR PLOTTING!!!!!!!!!!!!!!!!!!!!!!!!!! 
     ...
     if (time.eq.0.0_wp) then
        ...
        if (myid==0) then
           call ha_root(temp,temp_ha,deltax,mynx(ng),myny(ng),sxall,syall,&
                mynyall,npx,asp,numprocs,maxrow,comm2d)
           call ha_root_chem(chem,chem_ha,deltax,mynx(ng),myny(ng),sxall,&
                syall,mynyall,npx,asp,numprocs,comm2d)
           call ha_root(alpha,alpha_ha,deltax,mynx(ng),myny(ng),sxall,syall,&
                mynyall,npx,asp,numprocs,maxrow,comm2d)
           call ha_root(mu,mu_ha,deltax,mynx(ng),myny(ng),sxall,syall,&
           mynyall,npx,asp,numprocs,maxrow,comm2d)
        else
           call ha_rest(temp,deltax,mynx(ng),myny(ng),maxrow,comm2d)
           call ha_rest_chem(chem,deltax,mynx(ng),myny(ng),comm2d)
           call ha_rest(alpha,deltax,mynx(ng),myny(ng),maxrow,comm2d)
           call ha_rest(mu,deltax,mynx(ng),myny(ng),maxrow,comm2d)
        endif
...
!!!!!!!!!!!!!!!!!!!!!!!WRITE OUT TIME SERIES INFO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     if (mod(itime,ntdump) == 0.or.time.eq.0.0_wp) then
        call nusselt(temp(:,myny(ng)-1),dy(ng)%d(myny(ng)-1),deltax,asp,&
             nu,mynx,ng,topbdy)
        call usurf(u(:,myny(ng)-1),deltax,asp,u_surf,mynxu,ng,topbdy)
        if (topbdy) then
        ...
        call MPI_REDUCE(umin,umin_tot,1,MPI_DOUBLE_PRECISION,MPI_MIN,0,&
             comm2d,ierr)
        call rms(u,w,deltay,deltax,asp,v_rms,mynx,myny,ng)
        ...
        call average(tau,1.0_wp,deltay,deltax,asp,tau_avg,mynx,myny,ng)
        call average(tau_xx,2.0_wp,deltay,deltax,asp,tauxx_avg,mynx,myny,ng)
        !call average(tau_zz,2.0_wp,deltay,deltax,asp,tauzz_avg,mynx,myny,ng)
        call average(tau_xz,2.0_wp,deltay,deltax,asp,tauxz_avg,mynx,myny,ng)
        ...
        call average(temp,1.0_wp,deltay,deltax,asp,temp_avg,mynx,myny,ng)
        call average(alpha,1.0_wp,deltay,deltax,asp,alpha_avg,mynx,myny,ng)
        ! Calculate average chemical flux in cold downwellings 
        if (crust_production.or.chem_layer_init) then
           call chem_flux_avg(chem,temp,cfluxavg,deltax,deltay,mynx,myny,ng)
     ...
     dtd=h2/2
     dtc=(h2*lewis)/2
     dta=min(h/maxval(abs(u)),h/maxval(abs(w)))
     call MPI_ALLREDUCE(dta,dta_tot,1,MPI_DOUBLE_PRECISION,&
             MPI_MIN,comm2d,ierr)
     dt=min(dtd,dtc,dta_tot)*timestep
...
!!!!!!!!!!!!!!!!!!!!!!Tracer Advection!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     if (ntracers>0) then
        ...
        do i=1,myntracers
           ...
           if (xnew>myxmax.and.ynew>myymax.and.xnew<asp-deltax) then
              ...
              count_tr=count_tr+1
           elseif (xnew>myxmax.and.ynew>myymax.and.xnew>=asp-deltax) then
              my_tracers_out(8)=my_tracers_out(8)+1
              tr_out_topleft(count_tl)=i
              count_tl=count_tl+1
           elseif (xnew>myxmax.and.ynew<myymin.and.xnew<asp-deltax) then
           ...
           elseif (xnew>myxmax.and.ynew<myymin.and.xnew>=asp-deltax) then
              ...
              count_bl=count_bl+1
           elseif (xnew<myxmin.and.ynew<myymin.and.xnew>0.0_wp+deltax) then
              my_tracers_out(6)=my_tracers_out(6)+1
              tr_out_botleft(count_bl)=i
              count_bl=count_bl+1
           elseif (xnew<myxmin.and.ynew<myymin.and.xnew<=0.0_wp+deltax) then
              ...
              tr_out_botright(count_br)=i
              count_br=count_br+1
           elseif (xnew<myxmin.and.ynew>myymax.and.xnew>0.0_wp+deltax) then
              my_tracers_out(8)=my_tracers_out(8)+1
              tr_out_topleft(count_tl)=i
              count_tl=count_tl+1
           elseif (xnew<myxmin.and.ynew>myymax.and.xnew<=0.0_wp+deltax) then
              ...
              tr_out_topright(count_tr)=i
              count_tr=count_tr+1
           elseif (xnew>myxmax.and.xnew<asp-deltax) then
           ...
           elseif (xnew>myxmax.and.xnew>=asp-deltax) then
              ...
              count_l=count_l+1
           elseif (xnew<myxmin.and.xnew>0.0_wp+deltax) then
              my_tracers_out(7)=my_tracers_out(7)+1
              tr_out_left(count_l)=i
              count_l=count_l+1
           elseif (xnew<myxmin.and.xnew<=0.0_wp+deltax) then
      ...
      do i=2,mynx(ng)-1
         do j=2,myny(ng)-1
            ...
            if (chem_avg>c_max.and.temp(i,j+1)<0.5_wp) then
               sum1=sum1+0.5_wp*(chem(i,j)*w(i,j)+chem(i,j-1)*w(i,j-1))*dx*dy
               sum2=sum2+dx*dy
            endif
         enddo
      enddo
...
!
    SUBROUTINE usurf(u,dx,asp,uavg,mynx,ng,tbdy) 
      ...
      if (tbdy) then
         do i=1,mynx(ng)-2
            sum=sum+0.5_wp*(u(i)**2.0_wp+u(i+1)**2.0_wp)*dx
         enddo
      endif
      call MPI_REDUCE(sum,sum_tot,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,&
           comm2d,ierr)
...
!
    SUBROUTINE rms(u,w,dy,dx,asp,v_rms,mynx,myny,ng) 
      ...
      do i=1,mynx(ng)-2
         do j=1,myny(ng)-2
            sum=sum+0.5_wp*(u(i,j+1)**2.0_wp+u(i+1,j+1)**2.0_wp)*dx*dy+&
                0.5_wp*(w(i+1,j)**2.0_wp+w(i+1,j+1)**2.0_wp)*dx*dy 
         enddo
      enddo
...
!
    SUBROUTINE average_u(phi,dy,dx,asp,avg,mynx,myny,ng) 
      ...
      do i=1,mynx(ng)-2
         do j=1,myny(ng)-2
            sum=sum+0.5_wp*(phi(i,j+1)+phi(i+1,j+1))*dx*dy 
         enddo
      enddo
...
!    
    SUBROUTINE average(phi,power,dy,dx,asp,avg,mynx,myny,ng) 
      ...
      IMPLICIT NONE 
      REAL(WP), DIMENSION(:,:), INTENT(IN) :: phi
      REAL(WP), INTENT(IN) :: dy,dx,asp,power
      REAL(WP), INTENT(OUT) :: avg
      INTEGER(I4B), DIMENSION(:), INTENT(IN) :: mynx,myny
      INTEGER(I4B), INTENT(IN) :: ng
      INTEGER(I4B) :: i,j
...
!
    SUBROUTINE ha_root_chem(phi,ha,dx,mynx,myny,sxall,syall,mynyall,&
      ...
      do j=1,myny-2
         sum=0.0_wp
         do i=2,mynx-1
            sum = sum + 0.5_wp*(phi(i,j+1)+phi(i,j))*dx
         enddo
         do i=1,numprocs
            displ(i)=npx*(syall(i)-1) + nint((sxall(i)-1)/real(nx/npx)) &
                 + (j-1)*npx 
...
!
    SUBROUTINE ha_rest_chem(phi,dx,mynx,myny,comm) 
      ...
      do j=2,myny-1
         sum=0.0_wp
         do i=2,mynx-1
            sum = sum + 0.5_wp*(phi(i,j)+phi(i,j-1))*dx
         enddo
         send=1
         call MPI_GATHERV(sum,send,MPI_DOUBLE_PRECISION,0,0,0,&
              MPI_DOUBLE_PRECISION,0,comm,ierr)
...
! 
    SUBROUTINE calc_chem_tr_ratio(tracers,chem,x,y,myntracers,nx,ny,sx,sy,&
      ...
      do n=1,myntracers
         ...
         nx1=floor((tracers(n)%d(2)+(1.0_wp/(2.0_wp*ny)))*ny+2.0_wp-sx)
         nx2=ceiling((tracers(n)%d(2)+(1.0_wp/(2.0_wp*ny)))*ny+2.0_wp-sx)
         ...
         if (tracers(n)%d(1)==1.0_wp.and.nx1/=nx2.and.ny1/=ny2) then
            n_tr_weight_dense(nx1,ny1)=n_tr_weight_dense(nx1,ny1)+&
                 ...
                 deltax)*max(0.0_wp,1.0_wp-&
                 (abs(tracers(n)%d(3)-y(ng)%d(ny1)))/deltay)
            ...
            n_tr_weight_dense(nx1,ny2)=n_tr_weight_dense(nx1,ny2)+&
                 max(0.0_wp,1.0_wp-(abs(tracers(n)%d(2)-x(ng)%d(nx1)))/&
                 deltax)*max(0.0_wp,1.0_wp-&
                 (abs(tracers(n)%d(3)-y(ng)%d(ny2)))/deltay)
...
!!$                    0.2_wp*visc_scale_factor2(i,j+1)
!!$               smooth2(i,j)=0.5_wp*visc_scale_factor2(i,j)+&
!!$                    0.125_wp*visc_scale_factor2(i-1,j)+&
!!$                    0.125_wp*visc_scale_factor2(i+1,j)+& 
!!$                    0.125_wp*visc_scale_factor2(i,j-1)+&
!!$                    0.125_wp*visc_scale_factor2(i,j+1)
...
!!$               visc_scale_factor2(i,j)=0.5_wp*smooth2(i,j)+&
!!$                    0.125_wp*smooth2(i-1,j)+0.125_wp*smooth2(i+1,j)+&
!!$                    0.125_wp*smooth2(i,j-1)+0.125_wp*smooth2(i,j+1)
