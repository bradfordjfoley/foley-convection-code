...
CONTAINS
  ...
  SUBROUTINE interpgen_tracer(uc,wc,interpgen_u,interpgen_w,x1,y1,x_u,x,&
       y_w,y,nx,ny,sx,sxu,sy,syw,periodic,asp)
    USE btype
    IMPLICIT NONE
    ...
    if (periodic) then
       ...
       nx1_u=floor(x1*(nx/asp)+3.0_wp-sxu)
       nx2_u=ceiling(x1*(nx/asp)+3.0_wp-sxu)
    endif
    nx1=floor(x1*(nx/asp)+(5.0_wp/2.0_wp)-sx)   
    nx2=ceiling(x1*(nx/asp)+(5.0_wp/2.0_wp)-sx) 
    ny1_w=floor(y1*ny+3.0_wp-syw)
    ny2_w=ceiling(y1*ny+3.0_wp-syw)
    ny1=floor(y1*ny+(5.0_wp/2.0_wp)-sy)
    ny2=ceiling(y1*ny+(5.0_wp/2.0_wp)-sy)
